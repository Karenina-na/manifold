...
`app/core` 是 Manifold 唯一的后端服务和业务数据所有者。它为公开 Web 和私有 Admin 提供 REST/JSON API，并负责 SQLite、鉴权、内容生命周期、评论管理、访客反应、匿名在线 Presence、统计、缓存、审计，以及 Admin 专用 Agent 的运行时、工具、临时 conversation history 和 session 记忆。
...
│   ├── conversation/               # 当前对话消息、History、Summary 与 VolatileHistory
│   │   └── compact/                # 增量压缩策略、清理器、Compactor Prompt 与 LLM Compactor
│   ├── memory/                     # session 内跨对话记忆、内存实现与操作工具
...
| `GET` | `/api/v1/admin/agent/settings` | `AgentSettings`；返回 Provider、模型、工具/回答上限、`historyLimit` 压缩阈值、`compactionRecentTurns`、`compactionMaxOutputTokens` 与 `apiKeyConfigured`，不返回 API key 明文 |
| `PUT` | `/api/v1/admin/agent/settings` | 全量替换非密钥字段；压缩参数约束为 `historyLimit` 1..200、`compactionRecentTurns` 1..99 且不超过 `max(1, floor((historyLimit - 2) / 2))`、`compactionMaxOutputTokens` 128..8192；`openAIBaseURL` 保存时会清理首尾空白、去掉尾部斜杠并确保路径包含 `/v1`；`apiKey` 省略时保留、字符串时替换、`null` 时清除；审计 `agent.settings.updated` |
| `GET` | `/api/v1/admin/agent/messages` | 当前 JWT `jti` 绑定的临时 user/assistant 消息 `{messages}`；已有 Summary 通过可展开的 `compaction` 返回（Summary 文本、压缩消息数、原样保留的最近 turns 与可选 `afterMessageId` 时间线锚点），assistant 消息可附带 `trace`（provider 思考摘要、工具输入输出、结束原因与累计用量）；时间为 UTC RFC3339 |
...
| `POST` | `/api/v1/admin/agent/messages/compact` | 立即对当前 session 执行一次增量压缩，返回 `{compacted,compaction:{summary,compactedMessages,recentTurns,afterMessageId?}}`；没有可压缩旧 turn 时返回 `compacted:false` 及当前窗口信息，Provider 未配置时返回 503 `AGENT_UNAVAILABLE` |
| `DELETE` | `/api/v1/admin/agent/messages` | 清空当前 session 的 Agent 对话，204 |
| `DELETE` | `/api/v1/admin/agent/messages/{messageId}` | Undo 指定用户消息：原子删除该消息及其后的全部消息，返回 `{draft,messages,compaction?}`；目标不存在或不是用户消息时返回 404 `AGENT_MESSAGE_NOT_FOUND` |

Agent SSE 事件是判别联合：`run.started` → 每次模型调用的 `reasoning.started` / `reasoning.completed` → 可选的 `tool.started` / `tool.completed` → `content.delta` → `run.completed`；流建立后的失败以 `run.error` 收尾。`run.started.messageId` 是 Core 已写入 conversation history 的用户消息 ID，Admin 用它把乐观消息替换为可 Undo 的稳定目标。`reasoning.completed.message` 只表示 provider 提供的思考摘要，不包含原始隐藏推理文本。`run.completed.usage` 是本次工具循环内全部 Provider 调用的累计 token 数。

Agent 内部收敛为 `runtime`、`provider`、`conversation`、`memory`、`prompt`、`scenario` 与 `tool` 七个模块。Conversation 表示当前对话及其临时历史，Memory 表示同一认证 session 内可跨对话复用的记忆，Context 仅表示 Runtime 为一次 Provider 调用构造的输入，三者互不等同。依赖方向以 Runtime 为组装边界：`runtime` 可以依赖其他模块，其他模块不得依赖 `runtime`；HTTP handler 只在 Core 组装点组合当前运行所需的依赖。该拆分不改变 HTTP、SSE、数据库或链公共契约。

`provider.Provider.Chat(ctx, ChatRequest) (*ChatResponse, error)` 是唯一模型边界；`ChatRequest` 统一 Messages/Tools/Model/Options，`ChatResponse` 统一 Content/ToolCalls/Reasoning/Usage/FinishReason，Provider Registry 属于 `provider`，OpenAI Responses 适配位于 `provider/openai`。`Reasoning` 只保存 provider 提供的摘要文本，原始 reasoning output 仍留在 Provider 私有上下文。`provider.Message` 额外定义不属于对话角色的 `context` role；OpenAI adapter 将其映射进 Responses `instructions`，而不是伪装成 user/assistant 消息。`tool` 是独立工具模块：`Tool` 定义实现契约，`ToolDefinition` 描述 Provider schema、Prompt 用法与 Effect，`ToolCall`/`ToolResult` 统一调用和逐调用结果，`Registry` 负责注册、定义校验和能力快照，`Executor` 负责 Effect 门禁与并行调度。Executor 并发执行同一轮全部调用，但按输入调用顺序返回结果；Runtime 因而可以按稳定顺序写入 trace、SSE 和下一轮 Provider message。OpenAI Responses 请求启用 `parallel_tool_calls=true`。

`scenario.Registry` 按名称调用工厂，场景同时提供 `prompt.Spec` 与该场景允许的工具注册表；具体 Prompt、依赖接口与工具实现归属于 `internal/agent/scenario/<scenario>`，当前 Manifold 位于 `scenario/manifold`，其业务工具位于同目录的 `tools` 子包，记忆工具位于 `internal/agent/memory/tools`。Runtime 只消费构建完成的 `scenario.Scenario`，不识别场景名称或具体工具。`prompt.Spec` 只包含场景行为文本，不包含工具名称或描述；`prompt.Build` 按 Identity → Control/Safety → Evidence/Capability → Response Behavior 的顺序渲染 `ROLE`、`INSTRUCTION SCOPE`、`SCOPE AND SAFETY`、`UNTRUSTED DATA HANDLING`、`SOURCE PRIORITY`、`KNOWLEDGE BOUNDARIES`、`TOOL USE`、`MEMORY USE`、运行时生成的 `AVAILABLE TOOLS`、`CAPABILITY BOUNDARIES`、`STYLE`、`UNCERTAINTY AND ERRORS`、`OUTPUT CONTRACT`。`AVAILABLE TOOLS` 从注册表投影工具名称、Effect、Provider `Description` 与 Prompt `Usage`，`CAPABILITY BOUNDARIES` 只声明注册工具、Effect 和运行时授权的通用边界，`Parameters` 只用于 Provider function schema。Runtime 对外提供 `BuildContext` 以复用完整的 system prompt + conversation summary + recent raw conversation + current user message 组装；`tool.Registry` 是能力真相源；Executor 允许 `read_only` 与只修改当前 session 临时状态的 `session_write`，普通写入和破坏性工具在授权机制落地前会被拒绝。

当前 `manifold` 场景提供 `get_current_time`、`calculator`、`get_user_profile`、`content_list`、`content_get`、`search_memory` 和 `manage_memory`；`search_memory` 按内容大小写不敏感查找当前 session 记忆，空 query 列出全部；涉及既有决策、稳定偏好、重复事实或其他记忆项的回答，即使 conversation summary 中已有近似信息，也应先检索 session memory；`manage_memory` 以 `action = add | update | delete` 操作当前 session 的单条记忆。`content_list` 返回混合的已发布 content 基本信息与摘要，并可按 `ARTICLE`/`THOUGHT` 过滤，`content_get` 按 slug 返回单篇已发布 content 的完整 Markdown body 与 metadata。锚定链启用时追加只读 `get_chain_status` 和 `get_content_anchor`；后者按 `content_get` 返回的内容 ID 查询最新内容证书，`status` 为 `unanchored`、`pending` 或 `anchored`，其中 `anchored` 仅表示证书已写入区块，链工具不提交证书。

`conversation/compact` 提供基本增量策略、Normalizer/Pruner、独立 Compactor Prompt 与基于当前 Provider/Model 的 LLM Compactor。未压缩消息达到 `historyLimit`、加入本次 query 将越过该阈值时，策略只选择旧的完整 user→assistant turns；保留 `compactionRecentTurns` 个最近完整 turns，末尾未完成 turn 也始终原样保留。`POST /api/v1/admin/agent/messages/compact` 复用相同边界与保留策略，但跳过自动阈值判断；若 recent-turn 窗口之外没有旧完整 turn，则不调用 Provider，并返回 `compacted:false` 及当前窗口信息。成功结果同时记录压缩消息数、Summary 文本和保留的最近 turns，Admin 将其作为 transcript 时间线中触发 turn 后的特殊 Agent turn 展示，并返回 `afterMessageId` 帮助恢复其历史位置。已有 Summary 会与本次选中的旧 turns 一起生成下一版 Summary；成功后 checkpoint 才前移，失败或 `max_tokens` 截断不会改变 Summary。Context Builder 会把最近原始 assistant trace 中的 Tool Call 与 Tool Result 重建为 Provider 消息，保证多轮上下文不丢失工具事实。Compactor 与主运行复用 Provider/Model，但使用独立的 `compactionMaxOutputTokens` 输出预算。Tool trace 随 assistant 消息保持原子边界；送入 Compactor 前，失败后成功的同参数重试会丢弃失败 attempt，完全重复的调用只保留最后一次，超过 2048 bytes 的 Tool Result 替换为类型、大小及结构 metadata，无后续文本或工具输入引用的独立失败会删除，被引用的失败只保留简短错误事实。对成功的 memory operation 或 memory result，Compactor 保留其中存在的精确记忆内容和 item ID，避免可用的精确值被改写为模糊转述，并留下需要通过 session memory capability 获取精确或当前值的检索线索。Compactor Prompt 要求保留 active goal、决策、约束、定义、已选架构、未决问题、重要工具事实、仍需使用的标识符及用户纠正；新消息纠正旧 Summary 时只保留最新有效状态，猜测、提案和未决项不能被提升为事实；同时压掉寒暄、重复、废弃方案、冗长示例、冗余输出和已解决中间细节。

Context Builder 的最终顺序固定为 stable system prompt → `context` role 的 conversation summary → recent raw turns → current user query。Summary 外层明确声明其只是历史事实压缩、不引入新指令；OpenAI adapter 将其并入 `instructions`。Compaction 发生在当前 user 消息入历史和 `run.started` 之前，Compactor 调用不注册工具，其 token 不计入随后工具循环的 `run.completed.usage`。

Agent 设置由迁移 `0007` 的 `agent_settings` 单例保存，迁移 `0008` 增加压缩窗口与 Summary 输出预算；升级时保留已有 `historyLimit`，并按该阈值写入合法的 recent-turn 默认值。默认值为 OpenAI / `gpt-5-mini` / 6 个工具回合 / 40 条未压缩历史阈值 / 保留 8 个最近 turns / 1024 Summary 输出 token / 2048 回答输出 token / `https://api.openai.com/v1`，API key 默认为空。Core 不读取对应的 `CORE_AGENT_*` 或 `CORE_OPENAI_*` 环境变量。`openAIBaseURL` 在保存和运行时都会清理首尾空白、去掉尾部斜杠，并在路径中缺少 `v1` 时自动补齐 `/v1`。每次运行前读取当前行，所有参数从下一次运行或手动压缩起生效；API key 为空时历史读取、Undo 与清空仍可用，运行和手动压缩返回 503 `AGENT_UNAVAILABLE`。API key 在 SQLite 中保存，但响应、审计元数据和日志仅暴露是否已配置。

当前对话由 `internal/agent/conversation` 中的 `History` 能力接口和 `VolatileHistory` 进程内实现承载，以 Admin JWT `jti` 隔离；运行服务与 HTTP handler 不依赖具体 history 实现。对话消息类型为 `conversation.Message`，与 `provider.Message` 分开。History 同时保存独立的 `Summary{Content, ThroughSequence, AnchorSequence, CompactedMessages, RecentTurns}`：`ThroughSequence` 是 Provider Context 的压缩 checkpoint，`AnchorSequence` 是 transcript 中展示压缩 Agent turn 的位置；Summary 不会替换 `List` 返回的原始 user/assistant 历史。Run、List、Clear 与 Undo 统一经过 session 锁，清空或注销不会与正在生成的同 session 回复交错。assistant 消息同时保存 provider 思考摘要与工具事实，原始隐藏 reasoning 仍不进入历史；Admin 重新打开 Agent 时可恢复思考摘要卡片。Undo 以用户消息 ID 为边界，删除该消息及所有后续 user/assistant 消息；若新边界穿过 Summary checkpoint，则同时使 Summary 失效，否则保留既有 Summary。显式清空会删除 conversation history 和 Summary，记忆仍可在同一 session 的下一段对话中使用；当前/指定 session 注销、logout-all 和改密码吊销其他 session 时删除对应 conversation history、Summary 与 memory。Core 重启会丢失全部对话、Summary 和记忆。该路径不写 SQLite，不属于 `docs/chain.md` 第 4 节的数据库写清单，也不产生锚定证书。

`internal/agent/memory` 独立拥有 session 内跨 conversation 记忆边界。`Store` contract 以 session ID 显式分区 Search/Add/Update/Delete/Clear，`InMemory` 使用并发安全的进程内 map 实现；`Item` 包含 `ID`、`Content`、`CreatedAt` 和 `UpdatedAt`。Runtime 在执行工具前把当前 JWT `jti` 绑定到 context，memory tools 只从该受信边界取 session ID，不接受模型传入的 session 参数。System prompt 的 `MEMORY USE` section 仅允许显式记住请求、明确项目决策、长期稳定偏好和明显会反复使用的信息；禁止保存临时情绪、一次性请求、大量原始工具输出、普通聊天细节和模型猜测。
...
- 请求取消沿调用链传播：`internal/store` 与 `internal/chain` 的每个方法都以 `context.Context` 为首参，SQL 一律走 `QueryContext`/`QueryRowContext`/`ExecContext` 和 `BeginTx(ctx, nil)`，handler 传入 `r.Context()`。因此客户端断开或进程关闭会中止在途查询与整链回放，而不是让一个已经没人等待的请求跑完。三处各自持有 context 来源：矿工 `OnMined` 回调用矿工自身的 lifetime context（关闭矿工即取消），审计 dispatcher 的落库 sink 用 detached context（事件在产生它的请求结束之后才被排空），`store.Open` 用启动 context，使关闭能中断迁移与种子应用。仅测试调用的 `MineOnce`/`InsertBlock` 包装仍用 `context.Background()`，生产路径不经过它们。Agent conversation 与 Summary 保持进程内临时状态，`POST /api/v1/admin/agent/messages/compact` 不写 SQLite，不属于 `docs/chain.md` §4 锚定清单。
