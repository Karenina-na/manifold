...
| `GET` | `/api/v1/site` | 站点组合：profile 引用、站点身份（`title`/`description`/`footer`/`social`）、`commentsEnabled`、`featuredContent`（引用解析为已发布内容数组，未发布/缺失项跳过）、导航和 `sections`（枚举见下） |
...
| `GET/PATCH` | `/api/v1/admin/site` | 读取/更新站点设置（全量对象）：`title`（必填 ≤80）、`description`（≤200）、`footer`（≤200）、`social`（≤6 项，同导航项结构）、`commentsEnabled`、`featuredContent`（引用 ≤10）、`navigation`（1..10 项）与 `sections`（1..10 项，枚举 `PROFILE/BACKGROUND/RECENT_CONTENT/UPDATES/SERIES/CONTACT`，去重）；PATCH 校验失败返回 422，成功审计 `site.updated` |
