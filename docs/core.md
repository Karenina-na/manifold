...
| `GET` | `/api/v1/admin/agent/messages` | 当前 JWT `jti` 绑定的临时 user/assistant 消息 `{messages}`；已有 Summary 通过可展开的 `compaction` 返回（Summary 文本、压缩消息数、原样保留的最近 turns），assistant 消息可附带 `trace`（provider 思考摘要、工具输入输出、结束原因与累计用量）；时间为 UTC RFC3339 |
...
| `POST` | `/api/v1/admin/agent/messages/compact` | 立即对当前 session 执行一次增量压缩，返回 `{compacted,compaction:{summary,compactedMessages,recentTurns}}`；没有可压缩旧 turn 时返回 `compacted:false` 及当前窗口信息，Provider 未配置时返回 503 `AGENT_UNAVAILABLE` |
