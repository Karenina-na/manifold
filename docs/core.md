...
| `POST` | `/api/v1/admin/agent/messages/compact` | 立即对当前 session 执行一次增量压缩，保留配置的最近 raw turns；没有可压缩旧 turn 时仍返回 204，Provider 未配置时返回 503 `AGENT_UNAVAILABLE` |
