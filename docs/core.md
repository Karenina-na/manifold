...
| `POST` | `/api/v1/admin/agent/messages/compact` | 立即对当前 session 执行一次增量压缩，返回 `{compacted}`；没有可压缩旧 turn 时返回 `{compacted:false}`，Provider 未配置时返回 503 `AGENT_UNAVAILABLE` |
