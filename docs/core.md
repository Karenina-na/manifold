...
Agent Runtime 以 `Provider.Chat(ctx, ChatRequest) (*ChatResponse, error)` 为唯一模型边界；`ChatRequest` 统一 Messages/Tools/Model/Options，`ChatResponse` 统一 Content/ToolCalls/Usage/FinishReason。场景注册表按名称调用工厂，场景同时提供 system prompt 与该场景允许的工具注册表；Runtime 不包含 Manifold 业务提示词或工具选择。当前 `manifold` 场景提供 `get_current_time`、`calculator`、`get_user_profile`、`get_writings`、`get_thoughts`，锚定链启用时追加只读 `get_chain_status`。Context Builder 在场景 prompt 后拼接当前 session 最近消息和本次 user 消息；若消息数截断落在一轮对话中间，会丢弃开头孤立的 assistant 消息。内容工具只返回已发布内容的基本信息，不返回 Markdown body；链工具不提交证书。
