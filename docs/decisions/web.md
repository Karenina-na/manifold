...
两组内容在 Web 内混排，最多保留每类 1000 条历史副本；Recent Content 按发布时间倒序取每类最近 3 条，Updates 按不可变的 `publishedAt` 构建时间轨道。首个分页请求失败时首页显示 Core unavailable，后续历史页失败则保留已获取数据，不阻断主页。统计、发布状态和内容计数只使用 Core 返回值；任一首屏请求失败时，首页显示 Core unavailable 状态，不暴露内部错误。Now 状态数据源已随 Core 契约移除，首页不再请求 `now()`，Introduction 的 mood 状态徽标一并删除。
