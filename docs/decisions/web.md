...
首页六个区块标题上方的 eyebrow 小字固定为 `Profile`、`Background`、`Recent content`、`Updates`、`My Series`、`Contact`；Chain 页及其各节固定为 `Anchoring chain`、`Spine`、`Verify`、`Anchor`、`Ledger`、`Proof path`；评论区固定为 `Discussion`。这些小字不随 locale 切换，同名标题、正文、右侧 MinimalMetadata 提示和可访问标签仍使用本地化资源。
...
两个详情页（Writing 与 Thought）在标题面 meta 行（`articleMeta`）的行尾右下角渲染 `AnchorBadge`（`features/content/anchor-badge.tsx`，经 `ArticleMeta` 的 children / `ThoughtHeader` 的 `meta` slot 注入），由 `ContentDetail.latestAnchor` 驱动：无证书（`null`）不渲染；`pending` 显示指纹前缀 + "sealing…"（点击 `/chain`），`anchored` 显示指纹前缀 + 本地化块号（`block_3` → `Block #3` / `区块 #3`）并深链 `/chain?block=<blockId>`，chain 页据此选中 Sealed sequence 对应块（在 spine 窗口内）或展开账本行（旧块），并滚动定位；徽标状态色与 hover 由站点既有 accent token 提供，不引入新设计语言。
