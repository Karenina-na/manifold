...
| `/chain/explorer` | Dynamic Server Component + client explorer | `chain()`、`chainBlocks({page,pageSize:20})`、`chainAnchors({source,ref,page,pageSize:20})`、详情接口 | 统一浏览区块和证书。顶部页签切换 Blocks/Certificates，列表页码、来源/引用筛选和当前详情都写入 URL；点击列表项在同一页打开详情，区块详情可进入证书详情，证书详情可打开目标或跳转到 Tools 验证，并保留浏览器历史返回。 |
...
两个详情页（Writing 与 Thought）在标题面 meta 行（`articleMeta`）的行尾右下角渲染 `AnchorBadge`（`features/content/anchor-badge.tsx`，经 `ArticleMeta` 的 children / `ThoughtHeader` 的 `meta` slot 注入），由 `ContentDetail.latestAnchor` 驱动：无证书（`null`）不渲染；`pending` 显示指纹前缀 + "sealing…" 并进入 `/chain/tools?tab=submit`，`anchored` 显示指纹前缀 + 本地化块号（`block_3` → `Block #3` / `区块 #3`）并深链 `/chain/explorer?tab=blocks&block=<blockId>`，Explorer 以 URL 状态打开相应区块详情；Explorer 与 Tools 的页头返回入口在同源进入时使用浏览器历史，因此会保留来源页的归档筛选与滚动位置；徽标状态色与 hover 由站点既有 accent token 提供，不引入新设计语言。
