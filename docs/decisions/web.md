...
目录按边界组织：`app/` 只承载路由、布局和路由样式；`components/layout/` 是全局壳与 Provider，`components/ui/` 是跨域小组件；`features/` 下的 archive、chain、comments、content、home 各自聚合组件、状态、纯函数和同目录测试；`i18n/` 管理 locale 与翻译资源；`lib/` 只保留 Core client、观测、安全、主题和表达式求值等跨域基础设施。

Server Component 负责首屏数据、详情读取和 SEO；Client Component 负责评论、反应、导航菜单、命令式搜索、主题偏好、错误恢复和局部状态。顶部导航固定为居中 860px 毛玻璃容器，导航项由站点设置的 `navigation` 配置驱动（外链项新窗口打开；配置为空时回退内置 Home/Writings/Thoughts/Chain 四项），仍使用 route-aware pill；搜索通过现有 SDK 的 `content({ q, kind: ["ARTICLE", "THOUGHT"], pageSize: 8 })` 同时检索两类公开内容，并提供 Profile 的简历链接。主题偏好仅保存在浏览器 `localStorage` 的 `manifold.theme`，不改变 Core 数据或公共 API。界面语言支持英语与简体中文：根布局按 `manifold.locale` cookie → `Accept-Language` → 英语的优先级确定 SSR locale，`react-i18next` 管理 Client Component 文案；导航语言切换器先写入一年期 same-site cookie、更新客户端语言与 `document.documentElement.lang`，再 `router.refresh()` 让 Server Components 和 metadata 以同一 locale 重绘。切换器同时把当前值镜像到 `localStorage` 的同名键，供浏览器侧诊断与跨组件观察；SSR 与刷新后的权威来源仍是 cookie，存储不可用不阻断切换。归档状态 `In motion`、Chain 状态 `Mining`、Chain 概览协议字段 `Height`/`Commitments`/`Proof`/`Site key`，以及页脚的 `Powered by Manifold`/在线读者状态是固定产品文案，不随 locale 切换，也不进入翻译资源。首页右侧 `MinimalMetadata` 的区块名称与预览由 Server Component 使用同一 translator 生成，容器、跳转和可访问标签由 Client Component 的 `useLocale()` 生成；Writing/Thought 详情页的 metadata、返回入口和服务端标题使用 request locale，详情页客户端统计、评论、锚定状态与 `@manifold/render` 阅读控件则由根 Provider 接收同一 locale，因此正文控件、目录、脚注和日期不维护平行文案。英文本地化资源中的计数文案必须提供成对的 `_one`/`_other` key，调用方必须把 number（而不是预格式化字符串）作为 `count` 传给 i18next；`i18n/i18n-resources.test.mjs` 固定中英文 key parity、plural pair 完整性、固定产品文案不进入资源与关键页面文案可解析性。根布局是 async Server Component，除渲染全局 Chrome（导航、页脚、模拟终端）外，还并行通过 `loadSiteData()` 读取站点设置（`site()`）：`title`/`description` 驱动 `generateMetadata`（浏览器标题、`%s | 站点标题` 模板与默认 SEO 描述，读取失败回退内置 "Manifold" 文案），并与 `social` 一起构成终端用户目录中的站点说明和链接文件；`navigation`/`footer`/`social` 分别传给 SiteNav 与 SiteFooter。根布局不预取公开内容：`FloatingRepl` 仅在首次展开时经浏览器 SDK 并行惰性遍历 ARTICLE 与 THOUGHT 的公开分页（`pageSize: 100`），依据 Core 返回的 `pagination.totalPages` 挂载两类全部公开内容，失败时允许下次展开重试。`features/home/terminal-filesystem.ts` 把响应映射为只读的模拟 macOS 文件系统：根目录提供 `/Applications`、`/Library`、`/System`、`/Users`，`/Users/manifold` 下的 `writings/`、`thoughts/`、`links/`、`about.md` 与 `now.txt` 由当前站点配置和公开内容生成；命令共享同一路径解析和结果模型，`open` 只打开文件已绑定的公开 URL。终端窗口使用 macOS 标题栏与 zsh prompt，桌面居中、窄屏全屏，沿用 ⌘/Ctrl+J、Escape、Tab 补全、历史导航与 Ctrl/⌘+L；作为模态表面复用 `useModalFocus` 限制焦点并在关闭后归还。站点设置中 `commentsEnabled=false` 时，Writing/Thought 详情页（`loadSiteData()` 读取开关）不渲染讨论面与添加评论面；Core 同时在公开评论接口返回 403 兜底。Radix Theme 根节点使用 `hasBackground={false}`，由 Web 的 `--surface-paper` 统一管理页面背景，避免第三方主题默认白色背景形成横向色带。

首页六个区块标题上方的 eyebrow 小字固定为 `Profile`、`Background`、`Recent content`、`Updates`、`My Series`、`Contact`；Chain 落地页及其子页固定为 `Anchoring chain`、`Chain map`、`Explore`、`Tools`、`Blocks`、`Certificates`、`Verify`、`Anchor`、`Proof path`；评论区固定为 `Discussion`。这些小字不随 locale 切换，同名标题、正文、右侧 MinimalMetadata 提示和可访问标签仍使用本地化资源。

主题是单一事实来源 `lib/theme.ts`（存储键 `manifold.theme`、事件名 `manifold:theme`、"非 dark 即 light" 的归一化、阻塞脚本与外部 store 读写对全部集中在此），避免此前导航与 REPL 各写一份 `localStorage` 键、Radix `appearance` 却硬编码为 `light` 的三方不一致。根布局在 `<body>` 首个子节点渲染一段**阻塞式内联脚本**（`themeInitScript`），在首帧前把存储值写到 `document.documentElement.dataset.theme`，消除暗色读者的浅色闪烁；该脚本必须内联且早于应用 bundle，因此经 `contentSecurityPolicyNonce()` 从**请求头**读回 `proxy.ts` 下发的 nonce 并标注 `nonce={nonce}`（见第 6 节）。脚本执行后 DOM 属性——而非 `localStorage`——是运行时事实来源（REPL 的 `theme` 命令也只经 `applyTheme()` 写属性），React 侧由 `components/layout/theme-provider.tsx` 的 `useSyncExternalStore(subscribeToTheme, readCurrentTheme, serverTheme)` 读取，服务端快照固定为 `light`；`Providers` 内的 Radix `Theme` 由该 context 派生 `appearance`，导航开关与 REPL 命令都改走同一 `toggleTheme`/`applyTheme`，无需 effect 即可保持同步。因为该脚本在水合前改写了 `<html>` 自身的 `data-theme`，而服务端不可能知道读者的存储偏好，`layout.tsx` 的 `<html>` 标注 `suppressHydrationWarning`——这是保留预绘制脚本的官方做法，只覆盖该元素自身的属性，不覆盖子树；缺了它 React 会在**每次页面加载**都报 hydration mismatch（`browser-check` 的 `webErrors` 会直接判失败）。
...
| `/chain` | Dynamic Server Component | `chain()`、`chainBlocks({pageSize:5})`、`chainAnchors({pageSize:5})` | 轻量链入口：保留链状态概览、genesis/tip、最近区块与证书，并提供 Explorer 与 Tools 两个明确入口；旧的 `?block=<id>` 与 `?page=N` 查询会在服务端导向 Explorer，兼容详情页和外部旧链接。 |
| `/chain/explorer` | Dynamic Server Component + client explorer | `chain()`、`chainBlocks({page,pageSize:20})`、`chainAnchors({source,ref,page,pageSize:20})`、详情接口 | 统一浏览区块和证书。顶部页签切换 Blocks/Certificates，列表页码、来源/引用筛选和当前详情都写入 URL；点击列表项在同一页打开详情，区块详情可进入证书详情，证书详情可打开目标或跳转到 Tools 验证。页头固定链接回 `/chain`，详情区内的状态返回仍由 Explorer URL 状态控制。 |
| `/chain/tools` | Dynamic Server Component + client tools | `verify` 的 payload/hash/content/comment 入口、`submitAnchor`、pending 证书查询 | 将 Verify 与 Submit 收纳为两个页签；验证结果中的区块上下文可回到 Explorer，公开提交及本地 pending 记录保留原有冷却、限流和封存后查询行为。 |
...
两个详情页（Writing 与 Thought）在标题面 meta 行（`articleMeta`）的行尾右下角渲染 `AnchorBadge`（`features/content/anchor-badge.tsx`，经 `ArticleMeta` 的 children / `ThoughtHeader` 的 `meta` slot 注入），由 `ContentDetail.latestAnchor` 驱动：无证书（`null`）不渲染；`pending` 显示指纹前缀 + "sealing…" 并进入 `/chain/tools?tab=submit`，`anchored` 显示指纹前缀 + 本地化块号（`block_3` → `Block #3` / `区块 #3`）并深链 `/chain/explorer?tab=blocks&block=<blockId>`，Explorer 以 URL 状态打开相应区块详情；Explorer 与 Tools 的页头固定链接回 `/chain`，不依赖来源页或浏览器历史；徽标状态色与 hover 由站点既有 accent token 提供，不引入新设计语言。
...
Thoughts、Writings 两个归档和评论讨论面的 Previous/Next 分页器来自共享组件 `components/ui/pagination.tsx`（同一 `paginationSurface`/`pageButton`/`pageStatus` 样式类，仅 `aria-label` 与回调不同），单页（`totalPages <= 1`）时三个场景都隐藏分页器。
...
6. `role="dialog"` + `aria-modal="true"` 的模态表面必须同时具备焦点陷阱与焦点归还，二者由共享 `components/ui/use-modal-focus.ts` 的 `useModalFocus(active, containerRef)` 承担：激活时先把当时的 `document.activeElement` 记为 opener，再以**捕获阶段** `keydown` 监听把 Tab/Shift+Tab 折返到容器内首/末个可聚焦元素，卸载时把焦点还给仍连接的 opener。折返判定本身是零 DOM 依赖的纯函数 `focusWrapTarget()`（`components/ui/modal-focus.ts`，可聚焦选择器同文件导出），因此能在没有 jsdom 的环境里直接单测。hook 必须在"把焦点移入容器"的 effect **之前**调用，否则 opener 已被覆盖。当前只有两个模态表面：`features/content/article-lightbox.tsx` 的图片预览与 `components/layout/site-nav.tsx` 的 ⌘K 搜索面板；`components/ui/floating-tooltip.tsx` 是 `role="tooltip"` 的非模态浮层，**不得**套用该陷阱（`components/ui/modal-focus.test.mjs` 以结构化断言钉住这两点）。非模态的可点击容器（如 Admin 列表行）不走陷阱，改用 `role="button"` + `tabIndex={0}` + Enter/Space 键处理，并配套 `:focus-visible` 可见指示。
