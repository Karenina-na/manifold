...
在仓库根目录运行 `pnpm demo:pdf`，会用同一套 token 把三种常见的书面格式打成 PDF——同样的纸面、同样的衬线、同样的留白节奏。
...
只要这一行，颜色 / 字体 / 间距 token 就接入了——配合 [`CHEATSHEET.md`](./CHEATSHEET.md) 里的不变量使用即可。
...
| 路径 | 用途 |
|------|------|
| `src/tokens.css` | 颜色 / 字体 / 间距 token（Tailwind v4 `@theme`） |
| `SKILL.md` | AI 路由规则（mockup / 新组件 / mockup→React / token 审查） |
| `CHEATSHEET.md` | 一页速查：十条不变量 + 颜色 / 字号表 |
| `references/` | 完整规范（tokens / components / anti-patterns / mockup-to-react） |
| `templates/` | HTML mockup 起手式 |
| `showcase/` | 在线 showcase 源码 |

## 当作 AI Skill

[`SKILL.md`](./SKILL.md) 是写给 Claude Code / Codex 等 agent 的路由规则——当你说「做一个 Yohaku 风的 mockup」、「把这个 mockup 转成 React」或「检查 token 合规」时，它会把 `CHEATSHEET.md`、`references/`、`templates/` 里相应的上下文加载进来，让 agent 用同一套 token 产出页面。

最简单的接法是把这个目录拷到 agent 的 skill 路径下（例如 `~/.claude/skills/yohaku-design/`），然后在对话里直接说出触发词：
...
完整契约见 [`SKILL.md`](./SKILL.md) 与 [`CHEATSHEET.md`](./CHEATSHEET.md)。
...
更多见 [`CHEATSHEET.md`](./CHEATSHEET.md)。
...
- HTML 设计语言受到 [tw93/kami](https://github.com/tw93/kami) 的启发——那种以书写为中心、克制留白的版面气质，是 Yohaku 排版思路的重要起点。

---

## 许可

MIT — 见 `LICENSE`。仓库里 `assets/` 下的截图、对话归档以及本目录之外的散文内容，仍然采用 [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/)。
