...
async function main() {
  ...
  try {
    ...
    await replDialog.locator('[data-repl-index-status]').getByText(/Mounted \d+ writings and \d+ thoughts\./).waitFor({ state: 'visible' });
    await replInput.fill('cd w');
    await replInput.press('Tab');
    if (await replInput.evaluate((element) => element.value) !== 'cd writings/') throw new Error('Terminal did not complete a directory path with Tab');
    await replInput.fill('cat writings/re');
    await replInput.press('Tab');
    if (await replInput.evaluate((element) => element.value) !== 'cat writings/reading-the-edge.md') throw new Error('Terminal did not complete a content file with Tab');
    await replInput.fill('whoami');
    await replInput.press('Enter');
    if (!(await replDialog.locator('[data-repl-output]').last().textContent())?.includes('manifold')) throw new Error('Terminal whoami command did not return the simulated user');
    await replInput.fill('ls writings');
    await replInput.press('Enter');
    if (!(await replDialog.locator('[data-repl-output]').last().textContent())?.includes('designing-boundaries.md')) throw new Error('Terminal did not mount public writings');
    await replInput.fill('cat writings/designing-boundaries.md');
    await replInput.press('Enter');
    if (!(await replDialog.locator('[data-repl-output]').last().textContent())?.includes('/writing/designing-boundaries')) throw new Error('Terminal content file did not expose its public route');
    await replInput.fill('cd writings');
    await replInput.press('Enter');
    await replInput.fill('pwd');
    await replInput.press('Enter');
    if (!(await replDialog.locator('[data-repl-output]').last().textContent())?.includes('/Users/manifold/writings')) throw new Error('Terminal did not preserve its working directory');
    await replDialog.getByRole('button', { name: /Close command line/i }).click();
    const terminalTrigger = repl.getByRole('button', { name: /Open command line/i });
    await web.waitForFunction(() => document.activeElement?.getAttribute('aria-label') === 'Open command line');
    const searchTrigger = web.getByRole('button', { name: 'Search content' });
    ...
    await web.waitForFunction(() => document.activeElement?.getAttribute('aria-label') === 'Search content');
