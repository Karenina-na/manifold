...
import { formatRenderDate, type RenderLocale } from "./i18n/resources";
import { useRenderI18n } from "./render-i18n";

export function formatDate(iso: string, locale: RenderLocale = "en"): string {
  return formatRenderDate(iso, locale);
}
...
export function ThoughtHeader({ title, summary, date, mood, tags, actions, question, context, source, meta }: { title: string; summary?: string | null; date: string; mood?: string | null; tags: string[]; actions?: ReactNode; question?: string | null; context?: string | null; source?: string | null; meta?: ReactNode }) {
  const { locale, t } = useRenderI18n();
  return <header className="articleHeader">
    <span className="eyebrow">{t("thought")}</span>
    <h1>{title || t("thoughtFallbackTitle")}</h1>
    {summary && <p className="thoughtSummary"><span aria-hidden="true">✦</span>{summary}</p>}
    <div className="articleMeta" aria-label={t("thoughtMetadata")}>
      <span><CalendarDays size={14} aria-hidden="true" /> <time dateTime={date}>{formatRenderDate(date, locale)}</time></span>
  ...
  return <ReadingShell toc={toc} discussion={discussion}>
    <section className="articleTitleBlock">
      <header className="articleHeader">
        <span className="eyebrow">{t("writing")}</span>
        <h1>{title || t("writingFallbackTitle")}</h1>
        <p>{summary}</p>
        {meta}
      </header>
    </section>
