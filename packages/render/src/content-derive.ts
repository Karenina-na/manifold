...
export function deriveExcerpt(body: string): string {
  ...
  for (const line of body.replace(/\r\n/g, '\n').split('\n')) {
    let text = line.trim()
    if (text.startsWith('```') || text.startsWith('~~~')) { inFence = !inFence; continue }
    if (inFence || !text || /^\[[^\]]+\]:\s*\S+/.test(text)) continue
    text = text.replace(/^#{1,6}\s+/, '').replace(/^>\s?/, '').replace(/^(?:[-+*]|\d+[.)])\s+/, '')
      .replace(/!\[[^\]]*\](?:\([^)]*\)|\[[^\]]*\])/g, '').replace(/\[([^\]]+)\](?:\([^)]*\)|\[[^\]]*\])/g, '$1')
      .replace(/<((?:https?:\/\/|mailto:)[^>]+)>/g, '$1').replace(/<!--.*?-->/g, '').replace(/<\/?[A-Za-z][^>]*>/g, '')
      .replace(/(?:\*\*|__)(\S(?:.*?\S)?)(?:\*\*|__)/g, '$1').replace(/~~(\S(?:.*?\S)?)~~/g, '$1')
      .replace(/`([^`]+)`/g, '$1').replace(/\\([\\`*_[\]{}()#+.!<>~-])/g, '$1').trim().replace(/\s+/g, ' ')
...
export function estimateReadingMinutes(body: string): number {
  let latinWords = 0, cjkCharacters = 0, inWord = false
  for (const character of body) {
    if (isCJK(character)) { cjkCharacters++; inWord = false; continue }
    if (/[\p{L}\p{N}]/u.test(character)) { if (!inWord) latinWords++; inWord = true; continue }
    inWord = false
  }
  return Math.max(1, Math.floor((latinWords + Math.floor((cjkCharacters + 1) / 2) + 199) / 200))
}
...
export function deriveToc(body: string): Array<{ id: string; label: string; level: 2 | 3 }> {
  const entries: Array<{ id: string; label: string; level: 2 | 3 }> = [], usedIDs = new Map<string, number>()
  let inFence = false
  for (const line of body.split('\n')) {
    ...
    if (!match) continue
    const label = match[2].replace(/[*_`~]/g, '').trim(), baseID = slugifyHeading(label)
    if (!label || !baseID) continue
    let id = baseID
    for (let count = usedIDs.get(baseID) ?? 0; ; count++) {
      const candidate = count > 0 ? `${baseID}-${count + 1}` : baseID
      if (!usedIDs.has(candidate)) { id = candidate; usedIDs.set(baseID, count + 1); usedIDs.set(candidate, 1); break }
    }
...
function slugifyHeading(value: string): string {
  ...
  for (const character of value.toLowerCase()) {
    if (/[\p{L}\p{N}]/u.test(character)) { if (separator && slug.length > 0) slug += '-'; slug += character; separator = false } else separator = true
  }
