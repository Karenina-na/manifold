...
func (s *Store) Overview(ctx context.Context) (model.AdminOverview, error) {
	...
	createdBy, err := s.monthlyCounts(ctx, `SELECT strftime('%Y-%m', created_at), COUNT(*) FROM content WHERE status != 'DELETED' AND created_at >= ? GROUP BY 1`, monthStart.Format("2006-01-02"))
	if err != nil {
		return overview, err
	}
	publishedBy, err := s.monthlyCounts(ctx, `SELECT strftime('%Y-%m', published_at), COUNT(*) FROM content WHERE status = 'PUBLISHED' AND published_at IS NOT NULL AND published_at >= ? GROUP BY 1`, monthStart.Format("2006-01-02"))
	if err != nil {
		return overview, err
	}
	...
	overview.TopContent, err = s.topPublishedContent(ctx, 5)
	if err != nil {
		return overview, err
	}
	tags, err := s.Tags(ctx, "")
...
func (s *Store) monthlyCounts(ctx context.Context, query, from string) (map[string]int, error) {
...
func (s *Store) topPublishedContent(ctx context.Context, limit int) ([]model.AdminOverviewContentItem, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT id, kind, slug, title, view_count, like_count, comment_count FROM content WHERE status = 'PUBLISHED' ORDER BY view_count DESC, id ASC LIMIT ?`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) AnalyticsViews(ctx context.Context, days int) (model.AnalyticsViews, error) {
	if days <= 0 {
		days = 30
	}
	...
	if err := s.DB.QueryRowContext(ctx, `SELECT COUNT(*), COUNT(DISTINCT CASE WHEN visitor_id != '' THEN visitor_id END) FROM content_view_events WHERE day >= ?`, views.Range.From).Scan(&views.TotalViews, &views.UniqueVisitors); err != nil {
		return views, err
	}
	byDay, err := s.viewEventDaily(ctx, views.Range.From)
	if err != nil {
		return views, err
	}
	for i := 0; i < days; i++ {
	...
	views.Referrers, err = s.viewEventReferrers(ctx, views.Range.From, 10)
	return views, err
}

func (s *Store) viewEventDaily(ctx context.Context, from string) (map[string]model.AnalyticsDay, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT day, COUNT(*), COUNT(DISTINCT CASE WHEN visitor_id != '' THEN visitor_id END) FROM content_view_events WHERE day >= ? GROUP BY day`, from)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) viewEventReferrers(ctx context.Context, from string, limit int) ([]model.AnalyticsReferrer, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT CASE WHEN referrer = '' THEN 'direct' ELSE referrer END, COUNT(*) FROM content_view_events WHERE day >= ? GROUP BY 1 ORDER BY COUNT(*) DESC, 1 ASC LIMIT ?`, from, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
