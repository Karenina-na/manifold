...
func (s *Store) UpsertIdentity(ctx context.Context, provider, providerID, displayName, avatarURL, email string) (model.Identity, error) {
	id := "identity_" + provider + "_" + providerID
	now := nowRFC3339()
	if _, err := s.DB.ExecContext(ctx, `INSERT INTO identities (id, provider, provider_id, display_name, avatar_url, email, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
func (s *Store) GetIdentity(ctx context.Context, id string) (model.Identity, error) {
	var identity model.Identity
	err := s.DB.QueryRowContext(ctx, `SELECT id, provider, provider_id, display_name, avatar_url, email, created_at, updated_at FROM identities WHERE id = ?`, id).Scan(
