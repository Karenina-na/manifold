...
func (h *apiHandler) adminUploadMedia(w http.ResponseWriter, r *http.Request) {
	...
	if rawFilename == "" {
		WriteError(w, http.StatusUnprocessableEntity, apierror.ValidationError, "filename is required.")
		return
	}
	filename := store.SanitizeMediaFilename(rawFilename)
	if filename == "" {
		WriteError(w, http.StatusUnprocessableEntity, apierror.ValidationError, "filename is invalid.")
		return
	}
	...
	if err != nil {
		...
		if errors.As(err, &maxError) {
			WriteError(w, http.StatusRequestEntityTooLarge, apierror.MediaTooLarge, "Media exceeds the configured size limit.")
			return
		}
		WriteError(w, http.StatusBadRequest, apierror.MediaUnreadable, "Media body could not be read.")
		return
	}
	if len(data) == 0 {
		WriteError(w, http.StatusUnprocessableEntity, apierror.ValidationError, "Media body is empty.")
		return
	}
	mime := http.DetectContentType(data)
	if !allowedMediaMimes[mime] {
		WriteError(w, http.StatusUnsupportedMediaType, apierror.MediaTypeUnsupported, "Only PNG, JPEG, WebP, GIF, AVIF images and PDF files are accepted.")
		return
	}
...
func (h *apiHandler) adminDeleteMedia(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.mutations.DeleteMedia(r.Context(), mutationRequest(r), id); err != nil {
		...
		WriteError(w, http.StatusInternalServerError, apierror.MediaDeleteFailed, "Media could not be deleted.")
		return
	}
