...
func TestGitHubOAuthClientExchange(t *testing.T) {
	...
	defer server.Close()
	client := &githubOAuthClient{clientID: "test-id", clientSecret: "test-secret", redirectURI: "http://localhost:3000/api/v1/auth/callback/github", tokenURL: server.URL, apiURL: "https://api.github.com", http: server.Client()}
...
func TestGitHubOAuthClientExchangeError(t *testing.T) {
	...
	defer server.Close()
	client := &githubOAuthClient{tokenURL: server.URL, http: server.Client()}
	if _, err := client.exchange("nope"); err != errGitHubAuthFailed {
		t.Fatalf("expected errGitHubAuthFailed, got %v", err)
	}
}

// TestGitHubOAuthClientProfile verifies the profile leg: name falls back to
// login when empty, and a non-200 status surfaces errGitHubProfileMiss.
func TestGitHubOAuthClientProfile(t *testing.T) {
	...
	defer server.Close()
	client := &githubOAuthClient{apiURL: server.URL, http: server.Client()}
...
func TestGitHubOAuthClientProfileFailure(t *testing.T) {
	...
	defer server.Close()
	client := &githubOAuthClient{apiURL: server.URL, http: server.Client()}
	if _, err := client.profile("bad"); err != errGitHubProfileMiss {
		t.Fatalf("expected errGitHubProfileMiss, got %v", err)
	}
}
