...
func (ChainStatus) Definition() agent.ToolDefinition {
	return agent.ToolDefinition{Name: "get_chain_status", Description: "Get the current read-only anchoring-chain height, tip and anchor counts.", Parameters: json.RawMessage(`{"type":"object","properties":{},"required":[],"additionalProperties":false}`)}
}
