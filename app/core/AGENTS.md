...
Keep handlers, application services, domain models, and stores separated by responsibility. New database write paths must be recorded in the anchoring inventory or classified as an existing exception in `docs/chain.md`. Public API changes follow `contracts -> sdk -> core -> consumers -> docs -> tests`.
