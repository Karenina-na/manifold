...
`/thoughts` 由 Server Component 读取 Core 的 Thoughts aggregate，翻页时请求 Core 的对应页；置顶有效性、最新回退、置顶排除、正文摘录和页数均由 Core 负责，Web 只把当前页按月份/日期组织成纵向时间轴。Thoughts 与 Writings 归档都把星号灰色摘要和正文摘录分开，普通项限制两行正文、置顶项限制四行；归档页显示 Core 返回的点赞、观看和已审核评论数，不在 Web 端额外请求评论或计算统计。
