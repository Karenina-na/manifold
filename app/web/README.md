...
首页按 Profile/Introduction、Recent Content、Updates、Contribution activity、My Series、Contact 六段组织共同信息，展示语言参考仓库根目录的 `1.html`；Introduction 使用不透明 surface，Updates 使用最近 10 个内容的 `updatedAt`，同日更新合并为一个水平日期点，hover/focus 后在竖向预览中展示当天每条内容；Contribution activity 使用每类最多 1000 条公开内容的 `updatedAt`，可按年份切换并按 UTC 日期显示 0-4 级热度；页脚在线人数通过 Core presence 心跳返回，不使用 mock；页面数据、状态徽标和内容链接仍以 Core 返回值为准。
