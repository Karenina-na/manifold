...
test("the root layout does not pull the archive on every route", () => {
  ...
  const propsBlock = replSource.slice(replSource.indexOf("type FloatingReplProps"), replSource.indexOf("const COMMANDS"));
  assert.ok(propsBlock.length > 0, "the REPL props type should be discoverable");
  assert.doesNotMatch(propsBlock, /papers/, "the REPL must not receive papers as a prop");
  assert.match(replSource, /pageSize: 50/, "the REPL still requests the archive, lazily, once opened");
});
