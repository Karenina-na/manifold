...
export function SiteFooter({ footer, social }: { footer: string; social?: SiteNavigationItem[] }) {
  ...
  return <footer className={styles.siteFooter}>
    <div className={styles.siteFooterTop}>
      ...
      <span className={styles.footerPresence} aria-live="polite"><Circle size={7} fill="currentColor" aria-hidden="true" /> {activeVisitors === null ? "Counting readers" : `${activeVisitors} ${activeVisitors === 1 ? "reader" : "readers"} online`}</span>
    </div>
