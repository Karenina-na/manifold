...
import type { UpdateTimeline, UpdateTimelineMonth } from "./update-timeline";
...
function timelineDate(value: string, locale: Locale) {
  return new Intl.DateTimeFormat(locale, { month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(value));
}

function windowDateLabel(ts: number, locale: Locale) {
  return new Intl.DateTimeFormat(locale, { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }).format(new Date(ts));
}

function monthLabel(key: string, locale: Locale) {
  const [year, month] = key.split("-").map(Number);
  return new Intl.DateTimeFormat(locale, { month: "short", year: "2-digit", timeZone: "UTC" }).format(new Date(Date.UTC(year, month - 1, 1)));
}
...
type UpdateTimelineViewProps = { timeline: UpdateTimeline; hint?: string };

export function UpdateTimelineView({ timeline, hint }: UpdateTimelineViewProps) {
  const { t, i18n } = useTranslation();
  const locale = parseLocale(i18n.resolvedLanguage ?? i18n.language) ?? DEFAULT_LOCALE;
  const seasonLabels = [t("timeline.winter"), t("timeline.spring"), t("timeline.summer"), t("timeline.autumn")];
  ...
  const windowLabel = zoom === 0
    ? (timeline.truncated ? t("timeline.latest", { count: timeline.itemCount }) : t("timeline.allTime"))
    : windowDays >= 30
      ? t("timeline.lastMonths", { count: Math.round(windowDays / 30.44) })
      : t("timeline.lastDays", { count: windowDays });
  ...
  if (!timeline.points.length) return <p className={styles.muted}>{t("timeline.empty")}</p>;

  return <div ref={railRef} className={styles.updateTimeline} aria-label={t("timeline.label")}>
    <div className={styles.updateRailHeader}>
      <div><span className={styles.eyebrow}>Updates</span><h2 id="updates-heading" className={styles.updateTitle}>{t("common.sequence")}</h2></div>
      <div className={styles.updateRailActions}>
        ...
        <div className={styles.updateRangeWrap}>
          ...
          {rangeOpen && <div className={styles.updateRangePanel} role="dialog" aria-label={t("timeline.zoom")}>
            <span className={styles.updateRangeValues}>{windowDateLabel(startTs, locale)} – {windowDateLabel(endTs, locale)}</span>
            ...
            <div className={styles.updateRangeRow}>
              <span className={styles.updateRangeHint}>{timeline.truncated ? t("timeline.latestOf", { count: timeline.itemCount, total: timeline.totalItems }) : t("timeline.allHistory")}</span>
              <button className={styles.updateRangeReset} type="button" onClick={() => setZoom(100)}>{t("timeline.reset")}</button>
            </div>
          </div>}
        </div>
      </div>
    ...
    <div ref={trackRef} className={styles.updateTrack}>
      ...
      {seasonBands.map((band) => <span className={styles.updateSeasonBand} data-season={band.season} key={`${band.season}-${band.left}`} style={{ left: `${band.left}%`, width: `${band.width}%` }} aria-hidden="true">{band.width > 9 && <span className={styles.updateSeasonLabel}>{seasonLabels[band.season]}</span>}</span>)}
      {windowMonths.map((month, index) => (index % monthStep === 0 || index === windowMonths.length - 1 ? <span className={styles.updateMonth} data-update-month key={month.key} style={{ left: `${month.position}%` }}><span>{monthLabel(month.key, locale)}</span></span> : null))}
      {windowPoints.map((point, index) => {
        ...
        >
          <button
            ...
            aria-label={t("timeline.point", { count: point.updates.length, date: formatDate(point.date, locale, t("common.unpublished")) })}
          ...
          <div className={styles.updateDate} data-update-date data-hidden={showDate ? undefined : "true"}>{timelineDate(point.date, locale)}</div>
          <div className={styles.updatePreview} data-update-preview data-open={isOpen ? "true" : "false"} aria-hidden={!isOpen}>
            <span className={styles.updatePreviewMeta}>{t("timeline.preview", { count: point.updates.length, date: formatDate(point.date, locale, t("common.unpublished")) })}</span>
            <span className={styles.updatePreviewList}>
              {point.updates.map((entry) => <Link className={styles.updatePreviewItem} href={entry.href} key={entry.id} tabIndex={isOpen ? 0 : -1}>
                <span className={styles.updatePreviewItemMeta}>{(entry.kind === "ARTICLE" ? t("common.writing") : t("common.thought")).toLocaleUpperCase(locale)} · {formatDate(entry.date, locale, t("common.unpublished"))}</span>
    ...
    {!windowPoints.length && <p className={styles.muted}>{t("timeline.windowEmpty")}</p>}
  </div>;
}
