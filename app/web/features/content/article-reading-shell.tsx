...
import { resolveArticleActionsAtEnd } from "./article-end-threshold";
import { CommentComposer, CommentsPagingRefContext, ReplyContext, useReplyFocus, type CommentsPagingController, type ComposerPhase, type ReplyContextValue } from "../comments/comment-thread";
...
export function ArticleReadingShell({ children, discussion, toc, slug }: { children: React.ReactNode; discussion: React.ReactNode; toc: RenderTocItem[]; slug: string }) {
  ...
  return <CommentsPagingRefContext.Provider value={commentsPagingRef}><LayoutGroup id="article-reading-actions"><ReplyContext.Provider value={reply}>
    <ReadingShell
      toc={toc}
      rail={<aside className="articleActionRail" aria-label={t("detail.actions")}><AnimatePresence initial={false} mode="popLayout">{railVisible && <CommentComposer slug={slug} compact expanded={compactExpanded} onExpandedChange={setCompactExpanded} onPhaseChange={setRailPhase} />}</AnimatePresence></aside>}
      discussion={discussion}
      discussionTrigger={<div ref={discussionEndRef} className="articleComposerTrigger" aria-hidden="true" />}
      composer={<section className="articleComposerBlock" data-active={showBottomComposer ? "true" : "false"} aria-label={t("detail.addComment")} aria-hidden={!showBottomComposer}><AnimatePresence initial={false} mode="popLayout">{showBottomComposer && <CommentComposer slug={slug} expanded={bottomExpanded} anchorId="comment-composer" onExpandedChange={setBottomExpanded} onPhaseChange={setBottomComposerPhase} />}</AnimatePresence></section>}
    >
      {children}
    </ReadingShell>
