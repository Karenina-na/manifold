...
export function filterComments(comments: Comment[], search: string, filter: CommentFilter, now = Date.now()): Comment[] {
  ...
  const filtered = comments.filter((comment) => {
    const matchesSearch = !needle || (!comment.hidden && `${comment.authorName} ${comment.body}`.toLocaleLowerCase().includes(needle));
    if (!matchesSearch) return false;
    if (filter === "withWebsite") return !comment.hidden && Boolean(comment.authorUrl);
    if (filter === "recent") {
