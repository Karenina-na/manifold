...
export function LikeButton({ slug, compact = false }: LikeButtonProps) {
  ...
  return <div className={`${styles.likeBar} ${compact ? styles.likeBarCompact : ""}`} aria-label={t("like.label")}>
    <Button className={`${styles.likeButton} ${summary.viewerLiked ? styles.likeButtonActive : ""}`} variant="soft" type="button" aria-pressed={summary.viewerLiked} aria-label={summary.viewerLiked ? t("like.remove") : t("like.add")} onClick={() => toggle(!summary.viewerLiked)} disabled={mutation.isPending}>
      <Heart size={16} fill={summary.viewerLiked ? "currentColor" : "none"} /> <span>{summary.likeCount}</span>
    </Button>
    {query.isError && <span className={styles.likeError}>{t("like.unavailable")}</span>}
  </div>;
}
