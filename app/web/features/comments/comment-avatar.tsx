...
export function AvatarPicker({ seed, onChange }: { seed: string; onChange: (seed: string) => void }) {
  ...
  return <div className={styles.avatarPicker} ref={containerRef}>
    <button type="button" className={styles.avatarPickerTrigger} aria-haspopup="dialog" aria-expanded={open} aria-label={t("avatar.choose")} title={t("avatar.choose")} onClick={() => setOpen((value) => !value)}>
      <CommentAvatar seed={seed} size={34} />
    </button>
    <AnimatePresence initial={false}>
      {open && <motion.div className={styles.avatarPickerPanel} role="dialog" aria-label={t("avatar.choices")} initial={{ opacity: 0, y: -6, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.96 }} transition={{ duration: 0.16 }}>
        <div className={styles.avatarPickerGrid}>
          {candidates.map((candidate) => <button key={candidate} type="button" className={styles.avatarPickerOption} aria-pressed={candidate === seed} aria-label={t("avatar.use", { number: candidate.slice(-1) })} onClick={() => { onChange(candidate); setOpen(false); }}>
            <CommentAvatar seed={candidate} size={30} />
          </button>)}
        </div>
        <button type="button" className={styles.avatarPickerShuffle} onClick={() => setGeneration((value) => value + 1)}><Shuffle size={13} /> {t("avatar.shuffle")}</button>
      </motion.div>}
    </AnimatePresence>
  </div>;
}
