...
export function CommentsWorkspace({ token }: { token: string }) {
  ...
  const mutation = useMutation({ mutationFn: ({ id, action }: { id: string; action: 'delete' | 'restore' }) => action === 'delete' ? client.deleteComment(id) : client.restoreComment(id), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['admin-comments'] }); void queryClient.invalidateQueries({ queryKey: ['admin-stats'] }) } })
