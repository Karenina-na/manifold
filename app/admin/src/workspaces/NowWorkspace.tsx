...
export function NowWorkspace({ token }: { token: string }) {
  ...
  return <section className="workspace"><div className="page-heading"><div><p className="kicker">Presence</p><h1>What is alive now?</h1><p className="subheading">A small signal for what has your attention today.</p></div></div><section className="panel narrow-panel"><div className="panel-heading"><div><p className="kicker">Public beacon</p><h2>Now status</h2></div><Badge color="teal" variant="light" leftSection={<span className="status-dot published" />}>Live</Badge></div><form className="form-stack" onSubmit={form.handleSubmit((input) => mutation.mutate(input))}><TextInput label="Title" {...form.register('title')} placeholder="Building something small" error={form.formState.errors.title?.message} /><Textarea label="Detail" {...form.register('detail')} minRows={6} placeholder="What is taking shape?" /><TextInput label="Mood" {...form.register('mood')} placeholder="FOCUSED" error={form.formState.errors.mood?.message} /><Button className="button button-primary" type="submit" loading={mutation.isPending} leftSection={<Save size={16} />}>{mutation.isPending ? 'Updating...' : 'Update now'}</Button>{mutation.isSuccess && <p className="success-text">Now status updated.</p>}</form></section></section>
}
