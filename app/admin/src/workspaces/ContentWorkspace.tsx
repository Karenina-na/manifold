...
import type { AdminContent, ArticleMetadata, ContentInput, ThoughtMetadata } from '@manifold/contracts'
import { z } from 'zod'
...
export function ContentWorkspace({ token }: { token: string }) {
  ...
  const [filter, setFilter] = useState<'ALL' | AdminContent['kind']>('ALL')
  ...
  const transition = useMutation<AdminContent | void, Error, { id: string; action: 'publish' | 'unpublish' | 'delete' }>({ mutationFn: ({ id, action }) => action === 'publish' ? client.publishContent(id) : action === 'unpublish' ? client.unpublishContent(id) : client.deleteContent(id), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['admin-content'] }) })
