...
export function SettingsWorkspace({ token }: { token: string }) {
  ...
  const profileForm = useForm<ProfileForm>({ resolver: zodResolver(profileSchema), defaultValues: profileValues({ displayName: '', handle: '', headline: '', bio: '', avatarUrl: '', location: '', organization: '', websiteUrl: '', resumeUrl: '', interests: [], education: [], experience: [], id: 'profile_1', updatedAt: '' }) })
  ...
  const saveProfile = useMutation({ mutationFn: (input: ProfileForm) => client.updateProfile({ ...input, interests: input.interests.split(',').map((item) => item.trim()).filter(Boolean), education: JSON.parse(input.education), experience: JSON.parse(input.experience) }), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['admin-profile'] }) })
