...
import { toVditorLocale } from '../../i18n/locale'
...
export function MarkdownEditor({ value, onChange, disabled, placeholder, onUploadImage }: { value: string; onChange: (next: string) => void; disabled?: boolean; placeholder?: string; onUploadImage?: (file: File) => Promise<string> }) {
  ...
  const wrapSelection = useCallback((editor: Vditor, prefix: string, suffix: string, placeholder: string) => {
  ...
  const appendToEnd = useCallback((editor: Vditor, text: string) => {
    const irElement = editor.vditor.ir?.element
    if (!irElement) return
  ...
  const gfmToolbar = useCallback((editor: () => Vditor | null) => {
    const ed = () => editor()
    return [
      {
        ...
        name: 'callout', icon: calloutIcon, tip: t('markdown.callout'), click: () => {},
        toolbar: calloutVariants.map(({ type, icon }) => {
          const label = t(`markdown.calloutTypes.${type.toLowerCase()}`)
          return {
            name: `callout-${type.toLowerCase()}`,
            // Vditor renders level-2 items through Custom, which overwrites the
            // button content with `icon`, so the localized label travels inside it.
            icon: `${icon}<span style="margin-left:6px">${label}</span>`,
            tip: t('markdown.calloutType', { type: label }),
            click: () => {
      ...
      {
        name: 'footnote', icon: footnoteIcon, tip: t('markdown.footnote'),
        click: () => {
          const instance = ed(); if (!instance) return
      ...
      {
        name: 'kbd', icon: kbdIcon, tip: t('markdown.keyboard'),
        click: () => { const instance = ed(); if (instance) wrapSelection(instance, '<kbd>', '</kbd>', '⌘K') },
      },
      {
        name: 'mark', icon: markIcon, tip: t('markdown.highlight'),
        click: () => { const instance = ed(); if (instance) wrapSelection(instance, '<mark>', '</mark>', t('markdown.highlightPlaceholder')) },
      },
      {
        name: 'diff', icon: diffIcon, tip: t('markdown.diff'),
        click: () => {
          const instance = ed(); if (!instance) return
          instance.insertValue(`\`\`\`diff\n+ ${t('markdown.diffAdded')}\n- ${t('markdown.diffRemoved')}\n ${t('markdown.diffPlain')}\n\`\`\`\n`)
        },
      },
    ]
  }, [appendToEnd, t, wrapSelection])
  ...
  const uploadFiles = useCallback(async (files: File[]) => {
    ...
    for (const file of files) {
      if (!allowedImageTypes.has(file.type)) {
        editor.tip(t('markdown.unsupportedImage', { name: file.name || file.type }))
        continue
      }
      try {
        const url = await onUpload(file)
        const alt = file.name.replace(/[[\]()]/g, '').trim() || t('markdown.imageAlt')
        snippets.push(`![${alt}](${url})`)
      } catch (error) {
        editor.tip(error instanceof Error ? error.message : t('markdown.uploadFailed'))
      }
    }
  ...
  useEffect(() => {
    ...
    const editorOptions: IOptions & { mark?: boolean } = {
      ...
      toolbar: [
        ...
        ...(onUploadRef.current ? [{ name: 'upload-image', icon: uploadIcon, tip: t('markdown.uploadImage'), click: () => inputRef.current?.click() }] : []),
      ],
  ...
  }, [gfmToolbar, locale, t, uploadFiles])
  // The editor remounts when its localized toolbar changes; value changes flow through setValue.
